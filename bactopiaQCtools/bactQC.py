...
def check_bracken_results(sample_name, input_dir, min_primary_abundance=0.80):
    ...
    """
    # Get the file path
    file_path = os.path.join(input_dir, sample_name + '/tools/bracken/' + sample_name + '.bracken.tsv')
    ...
    if only_one_dominant_primary_species and not_ecoli_and_shigella:
        # Check if the first word within bracken_primary_species differs from the first word within the bracken_secondary_species
        bracken_result['Genus_conflict'] = bracken_result['bracken_primary_species'].str.split().str[0] == bracken_result['bracken_secondary_species'].str.split().str[0]
    else: bracken_result['Genus_conflict'] = False
    
    # Return True or False for the bracken_primary_species_abundance is greater than min_primary_abundance for first row
...
def check_mlst_results(sample_name, input_dir, expected_genus):
    ...
    """
    # Get the file path
    file_path = os.path.join(input_dir, sample_name + '/tools/mlst/' + sample_name + '.tsv')
    ...
    # Remove '.fna.fa' or '.fna' from the sample column
    ...
    # Download a text file mapping schemes and species from https://raw.githubusercontent.com/tseemann/mlst/refs/heads/master/db/scheme_species_map.tab
...
def check_checkm_results(sample_name, input_dir, min_completeness=0.80, max_contamination=10):
    ...
    Raises:
        ...
        ValueError: If multiple 'checkm.tsv' files are found, indicating ambiguity
            in selecting the appropriate file.
    ...
    # Check if we found more than one checkm.tsv file was found
    if len(checkm_files) > 1:
        raise ValueError("More than one checkm.tsv file found, using the most recent: {file_path}")
...
def check_fastp_data(sample_name, input_dir, min_q30_bases=0.80, min_coverage=30):
    ...
    # Get the file path
    file_path = os.path.join(input_dir, sample_name + '/main/qc/summary/' + sample_name + '.fastp.json')
    ...
    # Add a boolean for whether the Q30 bases are greater than min_q30_basesc
    fastp_results['passed_q30_bases'] = bool(fastp_results['post_filt_q30_rate'] > min_q30_bases)
    
    # Add a boolean for whether the coverage is greater than min_coverage
    fastp_results['passed_coverage'] = bool(coverage > min_coverage)
    
    # Add a boolean for whether the fastp QC passed
    fastp_results['passed_fastp_QC'] = bool(fastp_results['passed_q30_bases'] and fastp_results['passed_coverage'])
...
if __name__ == '__main__':  
        # Test the function
    sample_name = 'D7247039'
    input_dir = '/home/maxcu/RDH/Salm_Shig_typing/SAWGS_Typhi_or_Saintpaul/bactopia_out'
    df = check_bracken_results('D7247039', '/home/maxcu/RDH/Salm_Shig_typing/SAWGS_Typhi_or_Saintpaul/bactopia_out')
    df2 = check_checkm_results('D7247039', '/home/maxcu/RDH/Salm_Shig_typing/SAWGS_Typhi_or_Saintpaul/bactopia_out')

    df3 = check_mlst_results('D7247039', '/home/maxcu/RDH/Salm_Shig_typing/SAWGS_Typhi_or_Saintpaul/bactopia_out', 'Salmonella')
    
    
    #df4 = check_assembly_scan_results('D7247039', '/home/maxcu/RDH/Salm_Shig_typing/SAWGS_Typhi_or_Saintpaul/bactopia_out')
    df5 = check_fastp_data('D7247039', '/home/maxcu/RDH/Salm_Shig_typing/SAWGS_Typhi_or_Saintpaul/bactopia_out')
    #print(df)
    #print(df2) 
    #print(df3) 
